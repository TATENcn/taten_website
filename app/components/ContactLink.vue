...
<template>
  <NuxtLink :to="target" target="_blank" class="group relative h-[320px] border-b border-white overflow-hidden">
    <div class="relative h-full p-10 flex flex-col justify-between z-10">
      <div>
        <div class="text-[10px] text-red-600 font-bold mb-4 flex items-center gap-2">
          <span class="w-2 h-2 bg-red-600 animate-ping" />
          <span>[ {{ head }} ]</span>
        </div>
        <h3
      ...
      >
        <span class="text-[10px] font-bold uppercase opacity-40 italic">{{ what }}</span>
        <div
...
const what = defineModel<string>("what", { required: true });
const bottom = defineModel<string>("bottom", { required: true });
const description = defineModel<string>("description", { required: true });
const title = defineModel<string>("title", { required: true });
const head = defineModel<string>("head", { required: true });
</script>
